...
- **Single-company challenge**: The user asks about one ticker/company. Determine the exact value-chain position, evidence quality, direct and adjacent comparables, cross-industry analogues, what the market may be missing, and what would make the idea weak.
...
6. **Map relative chain position and comparables**
   - For single-company work, always place the company in a relative map before the final judgment.
   - Identify direct competitors, adjacent substitutes, key customers, key suppliers, and important private companies around the same layer.
   - Add cross-industry analogues only when the economic structure is genuinely similar: high qualification barriers, low supplier count, process control, installed-base lock-in, customer certification, or irreplaceable expansion role.
   - For each analogue, state why the comparison works, where it breaks, and what valuation or evidence lesson it gives.
   - Do not use analogues as proof. Use them to sharpen the research question and avoid false monopoly stories.

7. **Gather and grade evidence**
...
8. **Rank priorities**
   ...
   - For every final single-company answer, say whether the market is valuing the company more like a cyclical supplier, a process-specific equipment leader, a platform bottleneck, a customer-exposed beneficiary, or a theme story.
   - Use `scripts/serenity_scorecard.py` for repeatable scoring when Python is available and the user wants a score.

9. **Explain what could go wrong**
   - Describe the clearest situations that would show the idea is weak or wrong.
   - Cover substitution, faster competitor expansion, weak demand, dilution, poor margins, governance, geopolitics, customer loss, wrong analogy, and valuation already pricing in success.

10. **Give the next research move**
...
## Comparable chain positioning

Use this section for every single-company challenge and for final candidates in a deep theme scan when time allows.

The goal is to stop the answer from being a one-company story. Place the company on a map:

- **Exact layer**: the precise value-chain layer and process step.
- **Role type**: controls the scarce layer, supplies the scarce layer, benefits from the scarce layer, has weak pricing power, or mainly has a theme story.
- **Direct competitors**: companies competing for the same customer budget or process step.
- **Adjacent substitutes**: different technologies or process routes that could reduce the company's importance.
- **Customer chain**: downstream customers, OSATs, OEMs, foundries, cloud buyers, integrators, or capex owners that create demand.
- **Supplier chain**: upstream tools, materials, components, software, or IP that limit the company's own scaling.
- **Cross-industry analogues**: 1-3 companies in other industries with similar economics, not similar buzzwords.

For each comparable or analogue, answer:

`why the comparison works / where it breaks / what it teaches us about evidence or valuation`

Prefer this compact output when useful:

`direct competitors / adjacent substitutes / customer chain / cross-industry analogues / where the analogy breaks / next proof to check`

Rules:

- A direct competitor is stronger than a loose analogy.
- A customer or supplier relationship is not proof unless supported by filings, orders, qualification evidence, revenue concentration, or credible technical context.
- Never say "this is the ASML of X" without explaining the broken parts of the analogy.
- If the company has mixed businesses, map comparables by segment rather than treating the whole company as one pure-play.
