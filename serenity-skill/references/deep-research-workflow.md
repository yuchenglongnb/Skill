...
- place each final single-company candidate in a relative map of competitors, substitutes, customers, suppliers, and useful analogues when time allows;
...
### 7. Map relative chain position

For single-company work, and for the final candidates in a theme scan when time allows, add a relative map before the final judgment.

Use this checklist:

1. Exact layer and process step.
2. Role type: controls the scarce layer, supplies it, benefits from it, has weak pricing power, or mainly has a theme story.
3. Direct competitors at the same process step or customer budget.
4. Adjacent substitutes from different technologies, process flows, or integration choices.
5. Customer chain that creates demand.
6. Supplier chain that can limit the company's own scaling.
7. Cross-industry analogues with similar economics.

For each direct comparable or analogue, state:

- why the comparison works;
- where it breaks;
- what evidence or valuation lesson it gives.

Do not use analogues as proof. Use them to sharpen the research question. A phrase like "the ASML of X" is only allowed when the answer also explains the broken parts of the comparison.

For mixed-business companies, map comparables by segment. Do not let a legacy segment hide the economics of the scarce layer.
...
- Who are the direct competitors and adjacent substitutes?
- Which cross-industry analogue helps, and where does it fail?
